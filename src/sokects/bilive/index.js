...
export class BiliSocket {
    ...
    open(roomId, token, userId, hostList) {
        // const ws = new Socket('wss://broadcastlv.chat.bilibili.com:2245/sub');
        const url = hostList && hostList.length > 0 ? `wss://${hostList[0].host}:${hostList[0].wss_port}/sub` : 'wss://zj-cn-live-comet.chat.bilibili.com:2245/sub'
        ...
        ws.onopen = () => {
            ...
            this.roomId = rid;
            setTimeout(()=>{
                ws.send(encode.join(rid, token, userId));
            }, 100)
