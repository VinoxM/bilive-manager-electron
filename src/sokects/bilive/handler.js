    ...
    if (result.op === 5) {
        ...
        while (offset < buffer.length) {
            ...
            if (result.ver === 2) {
                ...
                let data = buffer.slice(offset + headerLen, offset + packetLen);
                try {
                    let newBuffer = zlib.brotliCompressSync(new Uint8Array(data));
...
export const encode = function (type, body) {
    ...
    let h = new ArrayBuffer(16)
    let d = new DataView(h, 0)
    let b = textEncoder.encode(body)
    d.setInt32(0, 16 + b.byteLength)
    d.setInt16(4, 16)
    d.setInt16(6, 1)
    if (type === 'heartbeat') {
        d.setInt32(8, 2)
    ...
    d.setInt32(12, 1)
    const result = mergeArrayBuffer(h, b)
    // console.log(result)
    return result
...
encode.join = (roomId, token, userId) => {
    return encode('join', {
        key: token,
        platform: 'web',
        protover: 2,
