        ...
        methods: {
            ...
            toggleConnect() {
                ...
                if (this.$ws.connected) {
                    ...
                    if (this.throughFlag) this.ipcRenderer.send('click-through-off')
                }
            },
            toggleOnTop() {
                ...
                if (this.roomTemp === 0) {
                    ...
                    this.$api.getRealRoomInfo(this.roomTemp).then(res => {
                        ...
                        if (roomInfo.roomId !== 0) {
                            this_.$api.getDanmuInfo(roomInfo.roomId, this_.register.cookie).then((r) => {
                                this_.$ws.open(roomInfo.roomId, r.token, this_.userInfo.uid, r.host_list)
            ...
            getRoomStream(roomId, qn) {
                ...
                return new Promise((resolve, reject) => {
                    const ops = []
                    this.$api.getRoomLiveInfo({cid: roomId, qn: qn}, this.register.cookie).then(res => {
                        console.log(res)
